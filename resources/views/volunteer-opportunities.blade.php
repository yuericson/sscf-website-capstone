...
<style>
  ...
  .volunteer-opportunity {
      ...
      text-align: left;
      height: 500px; /* Uniform height for all boxes */
      display: flex;
...
@php
    ...
    $opportunities = VolunteerOpportunities::orderBy('id', 'desc')->get();
@endphp

<!-- Volunteer Opportunities -->
<div class="volunteer-container my-5">
    <div class="text-center mb-4">
        <h2 class="volunteer-title">Volunteer Opportunities</h2>
        <div class="volunteer-underline mx-auto"></div>
    </div>

    <div class="row justify-content-center">
        @if($opportunities->count())
            @foreach ($opportunities as $opportunity)
                <div class="col-md-4 volunteer-opportunity" id="{{ Str::slug($opportunity->title, '-') }}">
                    @if($opportunity->image)
                        <img src="{{ asset('storage/' . $opportunity->image) }}" alt="{{ $opportunity->title }}">
                    @else
                        <img src="{{ asset('images/Volunteer.png') }}" alt="No Image Available">
                    @endif
                    <div class="opportunity-title">{{ $opportunity->title }}</div>
                    <div class="opportunity-description">{{ Str::limit($opportunity->content, 100, '...') }}</div>
                    <button class="view-more-button" data-bs-toggle="modal" data-bs-target="#modal-{{ $opportunity->id }}">View More</button>
                </div>

                <!-- Modal -->
                <div class="modal fade" id="modal-{{ $opportunity->id }}" tabindex="-1" aria-labelledby="modalLabel-{{ $opportunity->id }}" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="modalLabel-{{ $opportunity->id }}">{{ $opportunity->title }}</h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                            </div>
                            <div class="modal-body">
                                <img src="{{ asset('storage/' . $opportunity->image) }}" alt="{{ $opportunity->title }}" class="img-fluid mb-3">
...
<!-- JS for Highlighting Volunteer Opportunity -->
<script>
    document.addEventListener('DOMContentLoaded', function () {
        const urlParams = new URLSearchParams(window.location.search);
        const opportunitySlug = urlParams.get('opportunity');

        if (opportunitySlug) {
            const targetOpportunityBox = document.getElementById(opportunitySlug);

            if (targetOpportunityBox) {
                targetOpportunityBox.classList.add('highlighted-opportunity');
                targetOpportunityBox.scrollIntoView({ behavior: 'smooth', block: 'center' });
            }
        }
    });
</script>
