...
.latest-news-container {
  ...
  .latest-news-title h2 {
    font-size: 2.5rem;
    color: #185F43; /* Custom green color */
    font-weight: bold;
    margin-top: -50px;
  }
  ...
  .latest-news-underline {
    width: 120px;
    height: 4px;
    background-color: #185F43; /* Custom green color */
    margin: 0 auto 2rem auto;
  }

  .news-box {
    background-color: #f8f9fa; /* Light background for each news box */
    padding: 1rem;
    border-radius: 8px;
    box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);
    margin: 1rem; /* Margin between boxes */
    flex: 0 1 calc(30% - 2rem); /* Adjust to ensure equal spacing */
  ...
  .news-view-button:hover {
    background-color: #138A62;
  }

  /* CSS for Highlighting Selected News Box */
  .highlighted-news {
    border: 2px solid #185F43; /* Highlighting the selected news box */
...
@php
    ...
    // Fetch the latest news from the database (all news)
    $latestNews = LatestNews::orderBy('date', 'desc')->get();
@endphp

<!-- Latest News -->
<div class="latest-news-container my-5">
    <div class="container">
        <div class="latest-news-title text-center">
            <h2>Latest News</h2>
        </div>
        <div class="latest-news-underline text-center"></div>
        <div class="row justify-content-center">
            @if($latestNews->count())
                @foreach($latestNews as $news)
                    <div class="col-md-4 news-box d-flex flex-column" id="{{ Str::slug($news->title, '-') }}">
                        @if($news->image_path)
                            <img src="{{ asset('storage/' . $news->image_path) }}" alt="{{ $news->title }}" class="img-fluid mb-3 news-image">
                        @else
                            <img src="{{ asset('images/Volunteer.png') }}" alt="News Image" class="img-fluid mb-3 news-image">
                        @endif
                        <div class="news-content-wrapper mt-auto">
                            <div class="news-title">{{ $news->title }}</div>
                            <div class="news-date">Date: {{ Carbon::parse($news->date)->format('F d, Y') }}</div>
                            <div class="news-summary">
                                <p>{{ $news->content }}</p>
                            </div>
                            <button class="news-view-button" data-bs-toggle="modal" data-bs-target="#modal-{{ $news->id }}">View More</button>
                        </div>
                    </div>

                    <!-- Modal -->
                    <div class="modal fade" id="modal-{{ $news->id }}" tabindex="-1" aria-labelledby="modalLabel-{{ $news->id }}" aria-hidden="true">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="modalLabel-{{ $news->id }}">{{ $news->title }}</h5>
...
<!-- JS FOR HIGHLIGHTING NEWS IN LATEST NEWS -->
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (newsSlug) {
            const targetNewsBox = document.getElementById(newsSlug);

            if (targetNewsBox) {
                targetNewsBox.classList.add('highlighted-news');
                targetNewsBox.scrollIntoView({ behavior: 'smooth', block: 'center' });
            }
        }
    });
</script>
