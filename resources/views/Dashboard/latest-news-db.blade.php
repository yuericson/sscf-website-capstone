...
@php
    ...
    // Fetch all news for the dashboard with pagination
    $latestNews = LatestNews::orderBy('date', 'desc')->paginate(10);
...
<h2 class="mb-4 border-bottom pb-2">Latest News Dashboard</h2>
    @if(session('success'))
   ...
   <div class="row mt-4 mb-4">
    ...
    <div class="col-md-4 mb-4">
        <div class="d-flex align-items-center p-3 shadow rounded bg-white h-100">
            ...
            <div class="ms-3">
                <h6 class="text-muted mb-1">Latest News (Last 30 Days)</h6>
    ...
    <div class="col-md-4 mb-4">
        <div class="d-flex align-items-center p-3 shadow rounded bg-white h-100">
            ...
            <div class="ms-3">
                <h6 class="text-muted mb-1">Old News</h6>
                <h4 class="mb-0">{{ $oldNews }}</h4>
            </div>
        </div>
    </div>
</div>
    ...
    <div class="col-md-12">
        <div class="card">
            ...
            <div class="card-body" style="border: 1px solid #ccc; border-radius: 5px; padding: 20px; background-color: #f9f9f9;">
                <form action="{{ route('latest-news.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="row">
                        <!-- Title -->
                        ...
                        <div class="col-md-12 mb-3">
                            ...
                            <textarea name="content" id="content" rows="5" class="form-control" placeholder="Write the news content here..." required></textarea>
                        </div>
                        <!-- Image -->
                        <div class="col-md-6 mb-3">
        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center" style="background-color: #137547; color: white; padding: 10px;">
                    <h5 style="margin: 0;">Latest News</h5>
                </div>
                <div class="card-body">
    ...
    <tbody>
        @foreach ($latestNews as $news)
            <tr>
                ...
                <td style="text-align: center;">
                    @if($news->image_path)
                        <img src="{{ asset('storage/' . $news->image_path) }}" alt="{{ $news->title }}" style="width: 80px; height: 80px; object-fit: cover;" class="img-thumbnail">
                    @else
                ...
                <td>{{ $news->title }}</td>
                <!-- Date -->
                <td>{{ \Carbon\Carbon::parse($news->date)->format('F d, Y') }}</td>
                <!-- Content -->
                <td>{{ Str::limit($news->content, 50) }}</td>
                <!-- Actions -->
                <td class="text-center">
                    <button class="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#viewNewsModal{{ $news->id }}">View</button>
                    <button class="btn btn-sm btn-info" data-bs-toggle="modal" data-bs-target="#editNewsModal{{ $news->id }}">Edit</button>
                    <button class="btn btn-sm btn-danger" data-bs-toggle="modal" data-bs-target="#deleteNewsModal{{ $news->id }}">Delete</button>
                </td>
            </tr>
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="viewNewsModalLabel{{ $news->id }}">News Details</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            @if($news->image_path)
                                <div class="mb-3 text-center">
                                    <img src="{{ asset('storage/' . $news->image_path) }}" alt="{{ $news->title }}" class="img-thumbnail" width="300">
                                </div>
                            @else
                                <p class="text-center text-muted">No image available</p>
                            @endif
                            <div class="mb-3"><strong>Title:</strong> {{ $news->title }}</div>
                            <div class="mb-3"><strong>Date:</strong> {{ \Carbon\Carbon::parse($news->date)->format('F d, Y') }}</div>
                            <div class="mb-3"><strong>Content:</strong> {{ $news->content }}</div>
                        </div>
            ...
            <div class="modal fade" id="editNewsModal{{ $news->id }}" tabindex="-1" aria-labelledby="editNewsModalLabel{{ $news->id }}" aria-hidden="true">
                <div class="modal-dialog">
                    <form action="{{ route('latest-news.update', $news->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="editNewsModalLabel{{ $news->id }}">Edit News</h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                            </div>
                            <div class="modal-body">
                                <!-- Title -->
                                <div class="mb-3">
                                    <label for="edit_title_{{ $news->id }}" class="form-label"><strong>Title</strong></label>
                                    <input type="text" name="title" id="edit_title_{{ $news->id }}" class="form-control" value="{{ $news->title }}" required>
                                </div>
                                <!-- Date -->
                                <div class="mb-3">
                                    <label for="edit_date_{{ $news->id }}" class="form-label"><strong>Date</strong></label>
                                    <input type="date" name="date" id="edit_date_{{ $news->id }}" class="form-control" value="{{ $news->date }}" required>
                                </div>
                                <!-- Content -->
                                <div class="mb-3">
                                    <label for="edit_content_{{ $news->id }}" class="form-label"><strong>Content</strong></label>
                                    <textarea name="content" id="edit_content_{{ $news->id }}" rows="5" class="form-control" required>{{ $news->content }}</textarea>
                                </div>
                                <!-- Image -->
                                <div class="mb-3">
                                    <label for="edit_image_{{ $news->id }}" class="form-label"><strong>Image</strong></label>
                                    <div class="d-flex align-items-center">
                                        <div class="me-3">
                                            @if($news->image_path)
                                                <small><strong>Current Image:</strong></small><br>
                                                <img src="{{ asset('storage/' . $news->image_path) }}" alt="{{ $news->title }}" width="120" class="rounded border">
                                            @else
                                                <small><strong>No image available.</strong></small>
                                            @endif
                                        </div>
                                        <div>
                                            <input type="file" name="image" id="edit_image_{{ $news->id }}" class="form-control">
                                        </div>
                                    </div>
                                </div>
                            </div>
            ...
            <!-- Delete Modal -->
            <div class="modal fade" id="deleteNewsModal{{ $news->id }}" tabindex="-1" aria-labelledby="deleteNewsModalLabel{{ $news->id }}" aria-hidden="true">
                <div class="modal-dialog">
                    <form action="{{ route('latest-news.destroy', $news->id) }}" method="POST">
                        @csrf
                        @method('DELETE')
                        <div class="modal-content">
                            <div class="modal-header">
                            ...
                            <div class="modal-body">
                                Are you sure you want to delete <strong>{{ $news->title }}</strong>?
                            </div>
                            <div class="modal-footer">
                                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
        ...
        @endforeach
                        ...
                        <div class="d-flex justify-content-between align-items-center mt-3">
                            <p>Showing {{ $latestNews->firstItem() }} to {{ $latestNews->lastItem() }} of {{ $latestNews->total() }} results</p>
                            {{ $latestNews->onEachSide(1)->links('pagination::bootstrap-4') }}
                        </div>
                    @else
