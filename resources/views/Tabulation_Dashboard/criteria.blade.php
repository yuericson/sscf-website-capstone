...
<div class="main-content p-4" id="mainContent">
    <h2 class="mb-4 border-bottom pb-2">Criteria Dashboard</h2>
...
{{-- resources/views/partials/criteria.blade.php --}}
<div class="row mt-5">
    <div class="col-md-12">
        <div class="card mb-4">
            <div class="card-header d-flex justify-content-between align-items-center"
                 style="background-color:  #137547; color: white; padding: 10px;">
                <h5 class="mb-0">Criteria Management</h5>
                <button class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#addCriteriaModal">
                    Add Criteria
                </button>
            </div>
            <div class="card-body">
                        ...
                        @foreach($categories as $category)
                            @foreach($category->criteria as $criteria)
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <button class="btn btn-danger btn-sm" data-bs-toggle="modal"
                                                data-bs-target="#deleteCriteriaModal{{ $criteria->id }}">
                                            Delete
                                        </button>
                                    </td>
                                </tr>
                                ...
                                <div class="modal fade" id="editCriteriaModal{{ $criteria->id }}" tabindex="-1"
                                     aria-labelledby="editCriteriaModalLabel{{ $criteria->id }}" aria-hidden="true">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            <form action="{{ route('criteria-edit', $criteria->id) }}"
                                                  method="POST">
                                                @csrf
                                                @method('PUT')
                                                <div class="modal-header">
                                                    <h5 class="modal-title" id="editCriteriaModalLabel{{ $criteria->id }}">
                                                        Edit Criteria
                                                    </h5>
                                                ...
                                                <div class="modal-body">
                                                    <div class="mb-3">
                                                        <label for="name{{ $criteria->id }}" class="form-label">
                                                            Criteria Name
                                                        </label>
                                                    ...
                                                    <div class="mb-3">
                                                        <label for="weight{{ $criteria->id }}" class="form-label">
                                                            Weight (%)
                                                        </label>
                                ...
                                <div class="modal fade" id="deleteCriteriaModal{{ $criteria->id }}" tabindex="-1"
                                     aria-labelledby="deleteCriteriaModalLabel{{ $criteria->id }}" aria-hidden="true">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            <form action="{{ route('criteria-delete', $criteria->id) }}"
                                                  method="POST">
                                                @csrf
                                                @method('DELETE')
                                                <div class="modal-header">
                                                    <h5 class="modal-title" id="deleteCriteriaModalLabel{{ $criteria->id }}">
                                                        Delete Criteria
                                                    </h5>
                                                    <button type="button" class="btn-close" data-bs-dismiss="modal"
                                                            aria-label="Close"></button>
                                                ...
                                                <div class="modal-body">
                                                    Are you sure you want to delete <strong>{{ $criteria->name }}</strong>? This action cannot be undone.
                                                </div>
                                                <div class="modal-footer">
                                                    <button type="button" class="btn btn-secondary"
                                                            data-bs-dismiss="modal">
                ...
                @else
                    <p class="text-center mt-4">No Criteria Found.</p>
                @endif
            </div>
        </div>
    </div>
    ...
    <div class="modal fade" id="addCriteriaModal" tabindex="-1" aria-labelledby="addCriteriaModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                @if(Pageant::count() > 0)
                    <form action="{{ route('criteria-add', Pageant::first()->id) }}" method="POST" id="addCriteriaForm">
                        @csrf
                        <div class="modal-header">
                        ...
                        <div class="modal-body">
                            <div class="mb-3">
                                <label for="pageantSelect" class="form-label">Select Event</label>
                                <select class="form-select" name="pageant_id" id="pageantSelect" required>
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" name="category_id" id="categorySelect" required>
                                    @foreach(Category::with('pageant')->get() as $allCategory)
                                        <option value="{{ $allCategory->id }}">
                                            {{ $allCategory->name }} ({{ $allCategory->pageant->name }})
                                        </option>
                                    @endforeach
                                </select>
                                <small class="form-text text-muted">
                                    Select the category this criteria belongs to.
                                </small>
                            </div>
                            <div class="mb-3">
                                <label for="criteriaName" class="form-label">Criteria Name</label>
                                <input type="text" class="form-control" name="name" id="criteriaName" required>
                            </div>
                            <div class="mb-3">
                                <label for="criteriaWeight" class="form-label">Weight (%)</label>
                                <input type="number" class="form-control" name="weight" id="criteriaWeight" min="0" max="100" required>
                            </div>
                        </div>
                        <div class="modal-footer">
