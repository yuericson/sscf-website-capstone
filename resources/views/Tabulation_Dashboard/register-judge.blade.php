...
<div class="main-content p-4" id="mainContent">
    <h2 class="mb-4 border-bottom pb-2">Registered Judge Dashboard</h2>
...
{{-- resources/views/partials/registered-judges.blade.php --}}
<div class="row mt-3">
    <div class="col-md-12">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center"
                 style="background-color: #137547; color: white; padding: 10px;">
                <h5 class="mb-0">Registered Judges</h5>
            ...
            <div class="card-body">
                @if ($judges->count() > 0)
                    <table class="table table-bordered" id="judgesTable">
                        ...
                        @foreach ($judges as $judge)
                            <tr>
                                ...
                                <td>{{ $judge->email }}</td>
                                <td>
                                    @if($judge->pageants->count() > 0)
                                        <ul class="list-unstyled mb-0">
                                            @foreach($judge->pageants as $judgePageant)
                                                <li class="d-flex justify-content-between align-items-center mb-1">
                                                    {{ $judgePageant->name }}
                                                    <form action="{{ route('judge.removePageant', ['judge' => $judge->id, 'pageant' => $judgePageant->id]) }}"
                                                          method="POST">
                                                        @csrf
                                                        @method('DELETE')
                                    ...
                                    @else
                                        <p class="mb-0">No assignments.</p>
                                    @endif
                                </td>
                                <td>
                                    <button class="btn btn-warning btn-sm" data-bs-toggle="modal"
                                            data-bs-target="#editJudgeModal{{ $judge->id }}">
                                        Edit
                                    </button>
                                    <button class="btn btn-danger btn-sm" data-bs-toggle="modal"
                                            data-bs-target="#deleteJudgeModal{{ $judge->id }}">
                                        Delete
                                    </button>
                                    <button class="btn btn-secondary btn-sm" data-bs-toggle="modal"
                                            data-bs-target="#assignPageantsModal{{ $judge->id }}">
                                        Assign Event
                                    </button>
                                </td>
                            </tr>
                            ...
                            {{-- Edit Judge Modal --}}
                            <div class="modal fade" id="editJudgeModal{{ $judge->id }}" tabindex="-1"
                                 aria-labelledby="editJudgeModalLabel{{ $judge->id }}" aria-hidden="true">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <form action="{{ route('tabulation-judge-edit', $judge->id) }}"
                                              method="POST">
                                            @csrf
                                            @method('PUT')
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="editJudgeModalLabel{{ $judge->id }}">
                                                    Edit Judge
                                                </h5>
                                            ...
                                            <div class="modal-body">
                                                <div class="mb-3">
                                                    <label for="username{{ $judge->id }}" class="form-label">
                                                        Username
                                                    </label>
                                                    <input type="text" class="form-control" name="username"
                                                           id="username{{ $judge->id }}"
                                                           value="{{ old('username', $judge->username) }}" required>
                                                </div>
                                                <div class="mb-3">
                                                    <label for="name{{ $judge->id }}" class="form-label">
                                                        Name
                                                    </label>
                                                    <input type="text" class="form-control" name="name"
                                                           id="name{{ $judge->id }}"
                                                           value="{{ old('name', $judge->name) }}" required>
                                                </div>
                                                <div class="mb-3">
                                                    <label for="email{{ $judge->id }}" class="form-label">
                                                        Email
                                                    </label>
                            ...
                            {{-- Delete Judge Modal --}}
                            <div class="modal fade" id="deleteJudgeModal{{ $judge->id }}" tabindex="-1"
                                 aria-labelledby="deleteJudgeModalLabel{{ $judge->id }}" aria-hidden="true">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <form action="{{ route('tabulation-judge-delete', $judge->id) }}"
                                              method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="deleteJudgeModalLabel{{ $judge->id }}">
                                                    Delete Judge
                                                </h5>
                                                <button type="button" class="btn-close" data-bs-dismiss="modal"
                                                        aria-label="Close"></button>
                                            ...
                                            <div class="modal-body">
                                                Are you sure you want to delete <strong>{{ $judge->name }}</strong>?
                                            </div>
                                            <div class="modal-footer">
                                                <button type="button" class="btn btn-secondary"
                                                        data-bs-dismiss="modal">
                            ...
                            {{-- Assign Pageants Modal --}}
                            <div class="modal fade" id="assignPageantsModal{{ $judge->id }}" tabindex="-1"
                                 aria-labelledby="assignPageantsModalLabel{{ $judge->id }}" aria-hidden="true">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <form action="{{ route('judge.assign', $judge->id) }}" method="POST">
                                            @csrf
                                            <div class="modal-header">
                                                <h5 class="modal-title"
                                                    id="assignPageantsModalLabel{{ $judge->id }}">
                                                    Assign Event to {{ $judge->name }}
                                                </h5>
                                                <button type="button" class="btn-close" data-bs-dismiss="modal"
                                                        aria-label="Close"></button>
                                            </div>
                                            <div class="modal-body">
                                                <div class="form-group">
                                                    <label for="pageants{{ $judge->id }}">
                                                        Select Event:
                                                    </label>
                                                    <select name="pageant_ids[]" id="pageants{{ $judge->id }}"
                                                            class="form-control" multiple required>
                                                        @foreach(Pageant::all() as $allPageant)
                                                            <option value="{{ $allPageant->id }}"
                                                                {{ $judge->pageants->contains($allPageant->id) ? 'selected' : '' }}>
                                                                {{ $allPageant->name }}
                                                            </option>
                                                        @endforeach
                                                    </select>
                    ...
                    <div class="d-flex justify-content-between align-items-center mt-3">
                        <small>
                            Showing {{ $judges->firstItem() }} to {{ $judges->lastItem() }} of
                            {{ $judges->total() }} results
                        </small>
                        {{ $judges->links() }}
                    </div>
                @else
                    <p class="text-center mt-4">No Judge Accounts Found.</p>
                @endif
            </div>
        </div>
    </div>
    ...
    <div class="modal fade" id="addJudgeModal" tabindex="-1" aria-labelledby="addJudgeModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <form action="{{ route('tabulation-judge-add') }}" method="POST">
                    @csrf
                    <div class="modal-header">
