...
<div class="main-content p-4" id="mainContent">
    <h2 class="mb-4 border-bottom pb-2">Event Title Dashboard</h2>
...
{{-- resources/views/partials/event-title.blade.php --}}
<div class="row mt-5">
    <div class="col-md-12">
        <div class="card mb-4">
            <div class="card-header d-flex justify-content-between align-items-center"
                ...
                <button class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#addPageantModal">
                    Add Event
                </button>
            </div>
            <div class="card-body">
                @if($pageants->count() > 0)
                    <table class="table table-bordered" id="pageantsTable">
                        <thead>
                        <tr>
                            <th>ID</th>
                            <th>Name</th>
                        ...
                        @foreach($pageants as $pageant)
                            <tr>
                                <td>{{ $pageant->id }}</td>
                                <td>{{ $pageant->name }}</td>
                                <td>{{ ucfirst($pageant->gender) }}</td>
                                <td>{{ $pageant->description ?? 'N/A' }}</td>
                                <td>
                                    @if($pageant->judges->count() > 0)
                                        <ul class="list-unstyled mb-0">
                                            @foreach($pageant->judges as $assignedJudge)
                                                <li>
                                    ...
                                    @else
                                        <p class="mb-0">No judges assigned.</p>
                                    @endif
                                </td>
                                <td>
                                    <button class="btn btn-warning btn-sm" data-bs-toggle="modal"
                                            data-bs-target="#editPageantModal{{ $pageant->id }}">
                                        Edit
                                    </button>
                                    <button class="btn btn-danger btn-sm" data-bs-toggle="modal"
                                            data-bs-target="#deletePageantModal{{ $pageant->id }}">
                                        Delete
                                    </button>
                            ...
                            {{-- Edit Pageant Modal --}}
                            <div class="modal fade" id="editPageantModal{{ $pageant->id }}" tabindex="-1"
                                 aria-labelledby="editPageantModalLabel{{ $pageant->id }}" aria-hidden="true">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <form action="{{ route('pageant-edit', $pageant->id) }}"
                                              method="POST">
                                            @csrf
                                            @method('PUT')
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="editPageantModalLabel{{ $pageant->id }}">
                                                    Edit Event
                                                </h5>
                                            ...
                                            <div class="modal-body">
                                                <!-- Pageant Name -->
                                                <div class="mb-3">
                                                    <label for="name{{ $pageant->id }}" class="form-label">
                                                        Name
                                                    </label>
                                                    <input type="text" class="form-control" name="name"
                                                           id="name{{ $pageant->id }}"
                                                           value="{{ old('name', $pageant->name) }}" required>
                                                </div>
                                                <!-- Pageant Sex -->
                                                <div class="mb-3">
                                                    <label for="gender{{ $pageant->id }}" class="form-label">
                                                        Sex
                                                    </label>
                                                    <select class="form-select" name="gender"
                                                            id="gender{{ $pageant->id }}" required>
                                                        <option value="male"
                                                            {{ $pageant->gender == 'male' ? 'selected' : '' }}>
                                                            Male
                                                        </option>
                                                        <option value="female"
                                                            {{ $pageant->gender == 'female' ? 'selected' : '' }}>
                                                            Female
                                                        </option>
                                                        <option value="both"
                                                            {{ $pageant->gender == 'both' ? 'selected' : '' }}>
                                                            Both
                                                        </option>
                                                    </select>
                                                </div>
                                                <!-- Pageant Description -->
                                                <div class="mb-3">
                                                    <label for="description{{ $pageant->id }}" class="form-label">
                                                        Description
                                                    </label>
                                                    <textarea class="form-control" name="description"
                                                              id="description{{ $pageant->id }}"
                                                              rows="3">{{ old('description', $pageant->description) }}</textarea>
                                                </div>
                                                <!-- Pageant Date -->
                                                <div class="mb-3">
                                                    <label for="date{{ $pageant->id }}" class="form-label">
                                                        Date
                                                    </label>
                                                    <input type="date" class="form-control" name="date"
                                                           id="date{{ $pageant->id }}"
                                                           value="{{ old('date', \Carbon\Carbon::parse($pageant->date)->format('Y-m-d')) }}"
                                                           required>
                                                </div>
                                            </div>
                            ...
                            <div class="modal fade" id="deletePageantModal{{ $pageant->id }}" tabindex="-1"
                                 aria-labelledby="deletePageantModalLabel{{ $pageant->id }}" aria-hidden="true">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <form action="{{ route('pageant-delete', $pageant->id) }}"
                                              method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <div class="modal-header">
                                                <h5 class="modal-title"
                                                    id="deletePageantModalLabel{{ $pageant->id }}">
                                                    Delete Event
                                                </h5>
                                                <button type="button" class="btn-close" data-bs-dismiss="modal"
                                                        aria-label="Close"></button>
                    ...
                    <div class="d-flex justify-content-between align-items-center mt-3">
                        <small>
                            Showing {{ $pageants->firstItem() }} to {{ $pageants->lastItem() }} of
                            {{ $pageants->total() }} results
                        </small>
                        {{ $pageants->links() }}
                    </div>
                @else
                    <p class="text-center mt-4">No Event Found.</p>
                @endif
            </div>
        </div>
    </div>
    ...
    {{-- Add Pageant Modal --}}
    <div class="modal fade" id="addPageantModal" tabindex="-1" aria-labelledby="addPageantModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <form action="{{ route('pageant-add') }}" method="POST">
                    @csrf
                    <div class="modal-header">
                        <h5 class="modal-title" id="addPageantModalLabel">Add New Event</h5>
