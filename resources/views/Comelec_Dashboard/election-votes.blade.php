...
<div class="main-content p-4" id="mainContent">
    <h2 class="mb-4 border-bottom pb-2">Votes Management</h2>


@php
    // Retrieve votes with pagination
    $votes = \DB::table('election_voters_vote')
        ...
        ->paginate(20); // <-- use paginate(10) instead of get()

    // Transform each item in the pagination collection to include a formatted votes string
    $votes->transform(function ($vote) {
        ...
        $vote->formatted_votes = $formatted; // store this in a new property
        return $vote;
    });
@endphp
...
<div class="row mt-2">
    <div class="col-md-12">
        <div class="card">
            <div class="card-header" style="background-color: #137547; color: white; text-align: left; vertical-align: middle; padding: 15px;">
                <h5 style="margin: 0;">Votes Table</h5>
            </div>
            <div class="card-body">
                @if($votes->count() > 0)
                    <table class="table table-bordered">
                        <thead>
                            <tr>
                                ...
                                <th style="background-color: #D3D3D3; color: black;">Voter Name</th>
                                <th style="background-color: #D3D3D3; color: black;">Votes</th>
                                <th style="background-color: #D3D3D3; color: black;">Voted At</th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach ($votes as $vote)
                                <tr>
                                    ...
                                    <!-- Use the transformed property for formatted votes -->
                                    <td>{!! $vote->formatted_votes !!}</td>
                                    <td>{{ $vote->voted_at }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    ...
                    <div class="d-flex justify-content-between align-items-center mt-3">
                        <small>
                            Showing {{ $votes->firstItem() }} to {{ $votes->lastItem() }} of 
                            {{ $votes->total() }} results
                        </small>
                        <!-- You can customize the pagination view if desired, or just use links() -->
                        {{ $votes->onEachSide(1)->links('pagination::bootstrap-4') }}
                    </div>
                @else
                    <p class="text-center mt-4">No votes found.</p>
                @endif
            </div>
        </div>
    </div>
