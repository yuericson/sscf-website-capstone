...
.pre-registration-form {
    ...
    box-shadow: 0 8px 16px rgba(0, 0, 0, 0.2); /* Slightly darker shadow for depth */
    position: relative; /* Relative positioning for child elements */
    overflow: hidden; /* Hide overflow of child elements */
    color: #E1E289; /* Text color */
    background-color: #14591D; /* Background color */
...
.pre-registration-form::before {
    ...
    width: 100%;
    height: 100%;
    background: linear-gradient(rgba(41, 113, 38, 0.8), rgba(10, 33, 15, 0.8)); /* Soft pink to deep green */
...
.sports-title {
    ...
    color: #FCE7D9; /* Title color */
    font-family: 'Poppins', sans-serif; /* Clean and professional font for title */
}
...
.form-description {
    ...
    z-index: 1; /* Above gradient */
    color: #FCE7D9; /* Description color */
    font-family: 'Open Sans', sans-serif; /* Simple, clean font for description */
}

.btn-primary {
    font-size: 20px;
    padding: 10px 30px;
    background-color: #E1E289; /* Button background */
    border-color: #E1E289; /* Button border */
    margin-bottom: 50px;
    position: relative; /* Ensure button is above gradient */
    z-index: 1; /* Above gradient */
    color: #14591D; /* Button text color */
    font-family: 'Roboto', sans-serif; /* Consistent with the form font */
...
.btn-primary:hover {
    background-color: #FCE7D9; /* Hover background */
    border-color: #FCE7D9; /* Hover border */
    color: #0A210F; /* Hover text color */
}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function closeModal() {
    ...
    <style>
      /* Departments & Colleges Section - Scroll to the Right */
  .departments-section .logos-container {
    overflow: hidden;
    white-space: nowrap;
    width: 100%;
  }
  
  .departments-section .logos-wrapper {
    display: flex;
    width: calc(100% * 2);
    animation: scroll-right 50s linear infinite;
  }
  
  .departments-section .logos-inner {
    display: flex;
    flex-shrink: 0;
  }
  
  .departments-section .dc-logo {
    width: 250px;
    height: auto;
    margin: 0 20px;
    margin-top: 20px;
  ...
  /* Campuses Section - Scroll to the Left */
  .campuses-section .logos-container {
    overflow: hidden;
    white-space: nowrap;
    width: 100%;
  }
  
  .campuses-section .logos-wrapper {
    display: flex;
    width: calc(100% * 2);
    animation: scroll-left 50s linear infinite;
  }
  
  .campuses-section .logos-inner {
    display: flex;
    flex-shrink: 0;
  }
  
  .campuses-section .dc-logo {
    width: 250px;
    height: auto;
    margin: 0 20px;
    margin-top: 20px;
  ...
  .campuses-section .colleges-title {
    color: #003d00;
    background: linear-gradient(135deg, #d1f3d1, #b5e0b0);
  }
...
<div class="container my-5">
    <h3 class="text-center mb-4 title-heading">Select a Sport with Registered Participants</h3>
    <div class="row g-2 text-center">
        @php
            // Fetch distinct sports from the database
            $sports = \App\Models\SportsRegistration::select('sports_event')
                ->distinct()
                ->orderBy('sports_event', 'asc')
                ->pluck('sports_event');

            // Define Font Awesome icons and colors for each sport
            $sportIcons = [
                'Athletics' => ['icon' => 'fas fa-running', 'color' => 'text-warning'],
                'Arnis' => ['icon' => 'fas fa-user-ninja', 'color' => 'text-success'],
                'Archery' => ['icon' => 'fas fa-bullseye', 'color' => 'text-danger'],
                'Basketball' => ['icon' => 'fas fa-basketball-ball', 'color' => 'text-warning'],
                'Badminton' => ['icon' => 'fas fa-table-tennis', 'color' => 'text-primary'],
                'Volleyball' => ['icon' => 'fas fa-volleyball-ball', 'color' => 'text-warning'],
                'Billiards' => ['icon' => 'fas fa-circle-notch', 'color' => 'text-success'],
                'Beach Volleyball' => ['icon' => 'fas fa-umbrella-beach', 'color' => 'text-info'],
                'Chess' => ['icon' => 'fas fa-chess', 'color' => 'text-success'],
                'Dancesport (Standard)' => ['icon' => 'fas fa-music', 'color' => 'text-danger'],
                'E-Games ML' => ['icon' => 'fas fa-gamepad', 'color' => 'text-primary'],
                'Table Tennis' => ['icon' => 'fas fa-ping-pong', 'color' => 'text-warning'],
                'Lawn Tennis' => ['icon' => 'fas fa-table-tennis', 'color' => 'text-primary'],
                'Sepak Takraw' => ['icon' => 'fas fa-futbol', 'color' => 'text-success'],
                'Football' => ['icon' => 'fas fa-football-ball', 'color' => 'text-warning'],
                'Futsal' => ['icon' => 'fas fa-futbol', 'color' => 'text-success'],
                'Karatedo' => ['icon' => 'fas fa-hand-sparkles', 'color' => 'text-warning'],
                'Taekwondo' => ['icon' => 'fas fa-hand-rock', 'color' => 'text-success'],
                'Swimming' => ['icon' => 'fas fa-swimmer', 'color' => 'text-info'],
            ];
        @endphp

        @if ($sports->isEmpty())
            <p class="no-sports">No sports available.</p>
        @else
            @foreach ($sports as $sport)
                <div class="col-lg-2 col-md-3 col-sm-4 col-6">
                    <a href="{{ route('registrations.by.sport', $sport) }}" class="text-decoration-none">
                        <div class="sport-card">
                            <div class="card-body">
                                <div class="sport-icon {{ $sportIcons[$sport]['color'] ?? 'text-secondary' }}">
                                    <i class="{{ $sportIcons[$sport]['icon'] ?? 'fas fa-trophy' }}"></i>
                                </div>
                                <h5>{{ $sport }}</h5>
                            </div>
                        </div>
...
<style>
    ...
    .title-heading {
        ...
        font-weight: bold;
        color: #343a40; /* Dark gray for a clean look */
        text-transform: uppercase; /* Uppercase text for a formal style */
        position: relative;
        margin-top: 50px;
    }
    ...
    .title-heading::after {
        content: '';
        display: block;
        width: 150px; /* Underline width */
        height: 4px; /* Underline thickness */
        background: linear-gradient(90deg, #006400, #FFD700); /* SLSU green and gold gradient */
        margin: 8px auto 0; /* Center underline with spacing */
