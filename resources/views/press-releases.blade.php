 ...
 <!-- LOADER -->
  ...
  <div class="container my-5">
    @php
        ...
        // Siguraduhing mayroon tayong koleksyon kahit walang ipinasang $pressReleases mula sa controller
        $pressReleases = $pressReleases ?? PressReleases::orderBy('id', 'desc')->paginate(20);
    @endphp

    <div class="text-center mb-4">
        <h2 class="press-release-title">Press Releases</h2>
        <div class="press-release-underline mx-auto"></div>
    </div>
        ...
        @foreach ($pressReleases as $release)
            <div class="press-release">
                @if($release->image_path)
                    <img src="{{ asset('storage/' . $release->image_path) }}" alt="{{ $release->title }}" class="press-release-img">
                @else
                    <img src="{{ asset('images/Volunteer.png') }}" alt="No Image" class="press-release-img">
                @endif
                <div class="press-release-text">
                    <h4>{{ $release->title }}</h4>
                    <p class="date">Published on: {{ \Carbon\Carbon::parse($release->date)->format('F d, Y') }}</p>
                    <p>{{ Str::limit($release->content, 100, '...') }}</p>
                </div>
            </div>
        @endforeach
    </div>
    ...
    <div class="mt-5 text-center">
        <p class="press-release-call-to-action">
            <strong>Stay Updated with Our Latest News and Announcements!</strong>
            For more information about our initiatives, feel free to reach out to us.
        </p>
    </div>
</div>
...
<style>
    ...
    /* Additional Styling for Public Page */
    .press-release-title {
